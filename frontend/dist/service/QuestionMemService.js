...
class QuestionMemService {
    constructor() {
        if (localStorage.getItem("questions")) {
            let storedQuestions = JSON.parse(localStorage.getItem("questions")); //these question objects do not have the methods of a real Question object, because these methods are not contained in the JSON representation that is used for storage in the localStorage
            this.questions = [];
    ...
    getQuestionSync(id) {
        ...
        return (questionIndex !== -1) ? this.questions[questionIndex] : null;
    }
    ...
    getQuestions(ids, successCallback, errorCallback) {
        setTimeout(() => {
            let questions = [];
            for (let id of ids) {
                let question = this.getQuestionSync(id);
    ...
    createQuestion(question, successCallback, errorCallback) {
        setTimeout(() => {
    ...
    createQuestions(questions, successCallback, errorCallback) {
        setTimeout(() => {
            let ids = [];
    ...
    updateQuestion(question, successCallback, errorCallback) {
        setTimeout(() => {
    ...
    updateQuestions(questions, successCallback, errorCallback) {
        setTimeout(() => {
            for (let question of questions) {
                let questionIndex = this.getQuestionIndexSync(question.getId());
                if (questionIndex !== -1) {
    ...
    deleteQuestion(id, successCallback, errorCallback) {
        setTimeout(() => {
            let questionIndex = this.getQuestionIndexSync(id);
            if (questionIndex !== -1) {
    ...
    deleteQuestions(ids, successCallback, errorCallback) {
        setTimeout(() => {
            for (let id of ids) {
                let questionIndex = this.getQuestionIndexSync(id);
                if (questionIndex !== -1) {
...
//# sourceMappingURL=QuestionMemService.js.map