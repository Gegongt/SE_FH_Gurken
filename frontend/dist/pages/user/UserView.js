export class UserView {
    constructor() {
        ...
        if (!(picker instanceof HTMLInputElement))
            throw new Error("profilePicPicker not found");
        if (!(favList instanceof HTMLUListElement))
    ...
    renderProfilePicture(objectUrl) {
        const img = document.getElementById("profileImg");
        if (!img)
            return;
        if (!objectUrl) {
    ...
    bindProfilePicSelected(handler) {
        const input = document.getElementById("profilePicPicker");
        if (!input)
            throw new Error("profilePicPicker not found");
        input.onchange = () => {
    ...
    bindReportedFileActions(handler) {
        this.reportedFileList.addEventListener("click", (e) => {
            const btn = e.target.closest("button[data-action]");
            if (!btn)
                return;
            const action = btn.getAttribute("data-action");
            if (action !== "unreport" && action !== "delete" && action !== "block")
                return;
            ...
            if (!Number.isFinite(fileId) || !uploaderId)
                return;
            handler(action, fileId, uploaderId, fileName, uploaderName, uploaderPic);
        });
    }
    bindBlockedUserActions(handler) {
    ...
    renderBlockedUsers(users) {
        ...
        for (const u of users) {
            ...
            li.setAttribute("data-user-pic", u.getProfilePictureName?.() ?? u.profilepicturename ?? "");
            li.innerHTML = `
    ...
    bindBlockedUsersActions(handler) {
        this.blockedUsersList.addEventListener("click", (e) => {
            const btn = e.target.closest("button");
            if (!btn)
                return;
            if (btn.getAttribute("data-action") !== "unblock")
                return;
    ...
    bindFavouriteAction(handler) {
        this.favouritesList.addEventListener("click", (e) => {
            const btn = e.target.closest("button");
            if (!btn)
                return;
            const action = btn.getAttribute("data-action");
            if (action !== "download" && action !== "unfavourite")
                return;
            const li = btn.closest("li");
